...
export const STATEMENT_TYPES: readonly StatementType[] = ['事实', '推断', '偏好', '候选', '授权', '已验证结果']
...
/** 读取侧兼容：缺省陈述类型按「候选」解释 */
export function effectiveStatementType(e: MemoryEntry): StatementType {
...
function writeArchiveUnlocked(entries: MemoryEntry[]): void {
  const path = archivePath()
  mkdirSync(dirname(path), { recursive: true })
 ...
 */
function normalizeEntryEpistemics(entry: MemoryEntry): MemoryEntry {
 ...
 */
function evictionOrder(e: MemoryEntry): number {
  if (e.lifecycle?.state === '已替代') return 0
  const st = effectiveStatementType(e)
  if (st === '授权') return 3
  if (st === '候选' || st === '推断' || st === '偏好') return 1
  return 2
}
 ...
 */
function enforceActiveCapUnlocked(entries: MemoryEntry[]): MemoryEntry[] {
 ...
 * 归档区条目由调用方单独加载后并入。
 */
export function filterMemoriesForRead(entries: MemoryEntry[], options: ReadFilterOptions = {}): MemoryEntry[] {
  const { statementType, includeSuperseded = false } = options
  return entries.filter(e => {
    const state = effectiveLifecycle(e)
    if (state === '已归档') return false
    if (state === '已替代' && !includeSuperseded) return false
    if (statementType !== undefined && effectiveStatementType(e) !== statementType) return false
    return true
  })
}
...
export async function supersedeMemoryEntry(
  ...
  actor: MemoryActor = {},
): Promise<{ previous: MemoryEntry; current: MemoryEntry } | null> {
  const result = await withLock((): { previous: MemoryEntry; current: MemoryEntry } | null => {
    const entries = loadSharedMemory()
    ...
    const current = normalizeEntryEpistemics({
      ...
      statementType,
      lifecycle: { state: '当前' as LifecycleState, supersedes: id, reason: '修订' },
    })
...
export async function updateMemoryEntry(
  ...
  if (updates.content !== undefined || updates.statementType !== undefined || updates.type !== undefined) {
    const changes: SupersedeChanges = {}
    if (updates.content !== undefined) changes.content = updates.content
    if (updates.statementType !== undefined) changes.statementType = updates.statementType
...
export async function archiveMemoryEntry(id: string): Promise<boolean> {
  const result = await withLock((): boolean => {
    ...
    const archived: MemoryEntry = {
      ...removed,
      lifecycle: {
        ...(removed.lifecycle ?? { state: '当前' as LifecycleState }),
        state: '已归档',
...
export async function markMemorySuperseded(
  ...
  const result = await withLock((): boolean => {
    ...
    entry.lifecycle = {
      ...(entry.lifecycle ?? { state: '当前' as LifecycleState }),
      state: '已替代',
      supersededBy,
      reason,
    }
