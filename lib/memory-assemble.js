...
import { loadSharedMemory, filterMemoriesByUser, filterMemoriesForRead, searchMemories, searchMemoriesByKeywords, effectiveStatementType, effectiveSupport, effectiveLifecycle, } from "./memory-store.js";
...
export function assembleMemoryPack(viewer, query = {}) {
    ...
    // 关键词路径：searchMemoriesByKeywords 已按命中数降序排列 → 取头部（最相关）。
    // 无关键词路径：按存储序取最近（原语义）。
    // 2026-09-15 修复：此前统一 slice(-limit) 在关键词路径取的是「命中数最低的尾部」，
    // 导致最相关条目被切掉、分身拿错误记忆包做出错误判断（架构师文章实测实证）。
    const pack = query.keywords !== undefined && query.keywords.length > 0
...
function buildRelationDigest(viewer, packSize) {
    if (typeof viewer.actorId !== 'string' || viewer.actorId === '')
        return undefined;
    const actorId = viewer.actorId;
    const entries = loadSharedMemory().filter(e => e.relation !== undefined &&
...
export function registerAssembleApi(web, adminToken) {
    web.register({
        kind: 'exact',
        ...
        handler: (req, res) => {
            ...
            // token 门禁（与写路由同一门禁）：无令牌的本机/远端调用一律 401——
            // 防本机其他进程或 rebinding 页面自报 isMaster 套取主人记忆包
            if (r.headers['x-memory-token'] !== adminToken) {
                resLike.writeHead(401, { 'Content-Type': 'application/json' });
                resLike.end(JSON.stringify({ ok: false, error: '缺少或无效的校验 token（x-memory-token）' }));
                return;
            }
            const chunks = [];
            let size = 0;
            ...
            r.on('end', () => {
                try {
                    ...
                    const result = assembleMemoryPack(viewer, {
            ...
            r.on('end', () => {
                void (async () => {
                    try {
                        ...
                        const body = JSON.parse(chunks.map(c => c.toString('utf8')).join('') || '{}');
                        const m = await import("./memory-store.js");
