import { addMemoryEntry, loadSharedMemory, loadArchivedMemories, filterMemoriesByUser, filterMemoriesForRead, searchMemories, updateMemoryEntry, deleteMemoryEntry, effectiveStatementType, effectiveLifecycle, STATEMENT_TYPES, } from "./memory-store.js";
import { defineTool } from '@deepseek-ai/dsh-tools';
import { formatLocal } from './time-format.js';
const AUTH_STATUSES = ['未授权', '已授权', '已拒绝'];
const SOURCE_ORIGINS = ['seed', 'conversation', 'yuyi_message', 'tool_result', 'human', 'api'];
/** 渲染 JSON 结果为文本 */
 ...
 */
const viewerByCtx = new WeakMap();
/** 挂载点登记会话视角（registerMemoryTools 自动调用；一般无需手工调用）。 */
export function noteMemoryViewer(agentCtx, userId, isMaster) {
    if (agentCtx === null || typeof agentCtx !== 'object')
        return;
    viewerByCtx.set(agentCtx, { userId, isMaster });
}
/** 查询会话视角；未登记返回 undefined（调用方应 fail-closed 跳过）。 */
export function memoryViewerOf(agentCtx) {
    if (agentCtx === null || typeof agentCtx !== 'object')
        return undefined;
    return viewerByCtx.get(agentCtx);
}
/** 注册共享记忆工具到 agent 上下文 */
export function registerMemoryTools(agentCtx, userId, isMaster) {
    // 挂载即登记视角（web 预设行与 IM 驱动挂载都经过这里）：
    // 无论 tools 服务是否就绪，身份本身已经可信，自动装配/复盘据此过滤可见性。
    noteMemoryViewer(agentCtx, userId, isMaster);
    // memory_write 工具
    const writeTool = defineTool({
        ...
        execute: async (args) => {
            ...
            let scope = typeof args.scope === 'string' && ['master', 'self', 'public'].includes(args.scope)
                ? args.scope
                : (isMaster ? 'master' : 'self');
            // 访客不允许写 master-only 记忆（既无意义又易误导），统一回落为 self
            if (!isMaster && scope === 'master')
                scope = 'self';
            // 陈述类型：显式传入优先，否则主人=事实、访客=候选
            let statementType = typeof args.statementType === 'string' && STATEMENT_TYPES.includes(args.statementType)
                ? args.statementType
                : (isMaster ? '事实' : '候选');
            const notes = [];
            ...
            let auth;
            if (typeof args.authStatus === 'string' && AUTH_STATUSES.includes(args.authStatus)) {
                auth = {
            ...
            const source = {
                origin: (typeof args.sourceOrigin === 'string' && SOURCE_ORIGINS.includes(args.sourceOrigin)
                    ? args.sourceOrigin
    ...
    const readTool = defineTool({
        ...
        execute: async (args) => {
            ...
            const statementType = typeof args.statementType === 'string' && STATEMENT_TYPES.includes(args.statementType)
                ? args.statementType
                : undefined;
    ...
    const updateTool = defineTool({
        ...
        execute: async (args) => {
            ...
            if (typeof args.statementType === 'string' && STATEMENT_TYPES.includes(args.statementType)) {
