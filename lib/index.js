...
import { registerMemoryAutopilot, registerPackSection, reviewAllWindows } from "./memory-autopilot.js";
    ...
    try {
        ...
        const tryRegister = () => {
            ...
            if (attempts < 40)
                ...
                ctx.logger?.warn?.('[dsh-memory] systemPrompt 服务 10s 未就绪——按轮装配未注册（工具路径不受影响）');
        };
        tryRegister();
    }
    catch (error) {
        ctx.logger?.warn?.('[dsh-memory] 按轮装配接线失败（工具路径不受影响）:', error instanceof Error ? error.message : String(error));
    }
