...
export function effectiveSupport(e) {
 ...
 */
export function splitKeywords(text, max = 8) {
    ...
    const push = (raw) => {
        ...
        if (kw.length < 2 || kw.length > 24)
            return;
        if (KEYWORD_STOPWORDS.has(kw))
            return;
        if (seen.has(kw))
            return;
        seen.add(kw);
    ...
    // 拉丁/数字词元（含 @/#.- 等常见标识符字符）
    for (const m of trimmed.matchAll(/[A-Za-z0-9_@.#/\\-]{2,}/g))
        push(m[0]);
 ...
 */
export function searchMemoriesByKeywords(entries, keywords) {
    const kws = keywords
 ...
 */
export async function updateMemorySupport(id, update) {
    const result = await withLock(() => {
        const entries = loadSharedMemory();
        const entry = entries.find(e => e.id === id);
        if (entry === undefined)
 ...
 */
export async function addRelationEntry(input) {
...
export async function closeOpenLoop(memoryId, via = '主人确认') {
    const result = await withLock(() => {
        const entries = loadSharedMemory();
        const entry = entries.find(e => e.id === memoryId && e.relation !== undefined);
        if (entry === undefined || entry.relation?.openLoop === undefined)
            return null;
        if (entry.relation.openLoop.closedAt !== undefined)
            return entry;
...
/** 查询某对话者的未闭环事项 */
export function openLoopsForActor(actorId) {
    return loadSharedMemory().filter(e => effectiveLifecycle(e) === '当前' &&
