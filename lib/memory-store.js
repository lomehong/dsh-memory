...
/** 保存记忆（带文件锁） */
export function saveSharedMemory(entries) {
...
export function filterMemoriesByUser(entries, userId, isMaster) {
    ...
    return entries.filter(e => {
        ...
        if (e.scope === 'self' && e.author === userId)
            return true;
        if (e.scope === 'master' && e.participants?.includes(userId))
            return true;
        return false;
    });
}
...
/** 更新一条记忆的部分字段（按 id 查找） */
export function updateMemoryEntry(id, updates) {
    const entries = loadSharedMemory();
