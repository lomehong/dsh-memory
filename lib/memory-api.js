...
import { loadAutopilotConfig, saveAutopilotConfig } from "./memory-autopilot.js";
        ...
        handler: async (req, res) => {
            if (req && typeof req === 'object' && req.method === 'POST') {
                ...
                try {
                    ...
                    respondJson(res, 200, { ok: true, config: saved });
                }
                catch (error) {
                    respondJson(res, 500, { ok: false, error: messageOf(error) });
                }
            ...
            try {
                respondJson(res, 200, { ok: true, config: loadAutopilotConfig() });
            }
            catch (error) {
                respondJson(res, 500, { ok: false, error: messageOf(error) });
            }
